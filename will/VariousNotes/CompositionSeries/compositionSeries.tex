...
chain which has used all of the submodules\footnote{
In this case it turns out that we have a total order under inclusion of the
submodules of $M$. In general one does not need a condition this strong: it is
sufficient to have some maximal chain of submodules with respect to inclusion.
}
) and so the above chain is in fact a
